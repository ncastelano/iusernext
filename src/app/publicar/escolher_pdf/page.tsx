...
import { FaFilePdf, FaShare, FaArrowLeft } from "react-icons/fa";
...
export default function EscolherPDF() {
  ...
  const handlePublish = async () => {
    if (!pdfFile || pdfName.trim().length < 4) return;
    if (!position || !geohash) {
    ...
    try {
      const storageRef = ref(
        storage,
        `pdfpublication/${Date.now()}_${pdfFile.name}`
      );
  ...
  const canPublish = !!pdfFile && pdfName.trim().length >= 4 && !isPublishing;
    ...
    >
      ...
      >
        ...
        {pdfFile && (
          ...
          >
            <FaFilePdf style={{ fontSize: "32px", color: "#E74C3C" }} />
        ...
        <input
          type="text"
          placeholder="Digite o nome ou título do PDF..."
          value={pdfName}
